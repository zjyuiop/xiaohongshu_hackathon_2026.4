...
3. `selectedAgentIds` 只表示本场真正参会的人格，数量必须是 `2 ~ 3` 个。
4. 角色页进入会议页时，至少要把 `profile / nodes / agents` 或等价 bundle 带过来。
...
- `maxMessageChars = 180`
- `guidance = ''`
- `selectedAgentIds` 最多保留 3 个

其中，web 的角色选择策略是：

- 没选中过：直接加入
- 已选中过：取消选择
- 已经 3 个了还要再点第 4 个：把最早选中的踢掉，再加入新角色
...
| 状态 | 作用 |
|---|---|
| `selectedAgentIds` | 当前选中的 2~3 个参会人格 |
| `topic` | 当前议题 |
